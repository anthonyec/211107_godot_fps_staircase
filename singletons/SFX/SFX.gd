...
# Type of sounds is:
# "sound_name": ["sound_file.wav": AudioStream]
var sounds = {}

func _ready():
	print("SFX singleton started")
	...
	for result in results:
		...
		if number == 0:
			sounds[name.replace(number, "")] = [load(result)]
		else:
			sounds[name.replace(number, "")] = []
			
			for index in range(number):
				var path = result.replace(number, String(index + 1))
				sounds[name.replace(number, "")].append(load(path))
